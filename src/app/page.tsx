...
export default async function Home() {
  ...
  const [{ data: todos }, { data: habits }, { data: habitLogs }, { data: expenses }, { data: notes }] =
    await Promise.all([
  ...
  const monthLogCount = hl.filter(l => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-8 font-sans">
      ...
      <header className="mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="text-right hidden sm:block">
            <p className="text-white font-medium text-sm">{completedCount}/{t.length} 待办完成</p>
            <p className="text-slate-400 text-xs">本月支出 ¥{monthTotal.toFixed(0)}</p>
          </div>
      ...
      <div className="mb-6 rounded-xl bg-slate-800/50 border border-slate-700 p-3">
        <div className="flex flex-wrap items-center gap-4 text-xs">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-emerald-400">Supabase 已连接</span>
          </div>
          <div className="h-3 w-px bg-slate-600 hidden sm:block" />
          <div className="flex items-center gap-1.5 text-slate-300 hidden sm:flex">
            <span>📋</span><span>待办 {t.length}</span>
          </div>
          <div className="h-3 w-px bg-slate-600 hidden sm:block" />
          <div className="flex items-center gap-1.5 text-slate-300">
            <span>🎯</span><span>习惯 {h.length}</span>
          </div>
          <div className="h-3 w-px bg-slate-600" />
          <div className="flex items-center gap-1.5 text-slate-300">
            <span>📊</span><span>本周 {weekLogs.length} 次打卡</span>
          </div>
          <div className="h-3 w-px bg-slate-600" />
          <div className="flex items-center gap-1.5 text-slate-300">
            <span>💰</span><span>本周 ¥{weekTotal.toFixed(0)}</span>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-end gap-2 h-20">
            {weeklyData.map((w, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                ...
                <div
                  className="w-full bg-gradient-to-t from-rose-500/60 to-rose-400/40 rounded-t-sm transition-all hover:from-rose-400/80 hover:to-rose-300/60"
                  style={{ height: `${Math.max((w.total / maxWeekly) * 64, w.total > 0 ? 6 : 0)}px` }}
                />
      ...
      <footer className="mt-6 text-center text-slate-600 text-xs">
        Supabase Local · Next.js 16 · Tailwind CSS
      </footer>
    </div>
  )
}
